    ...
    with open(os.devnull, 'w') as FNULL:
        ...
        for idx in range(end_idx):
            ...
            for mpileup in pileup.stdout:
                ...
                if data:
                    if is_anno:
                        print_anno.print_data( data, w, min_depth, mismatch_rate_disease, mismatch_rate_normal, post_10_q, fisher_threshold, min_variant_read, flag_mis_base_0)
                    else:
                        print_vcf.print_data( data, w, min_depth, mismatch_rate_disease, mismatch_rate_normal, post_10_q, fisher_threshold, min_variant_read, flag_mis_base_0)
            pileup.stdout.close()  
            pileup.wait()
...
def Pileup_and_count(
    ...
    if len(region_list) > 0:
        ...
        for idx, target_regions in enumerate(region_list):
            proc = multiprocessing.Process(target = Pileup_command_multi_thread, \
                args = (target_regions, cmd_list, min_depth, min_variant_read, mismatch_rate_disease, mismatch_rate_normal, post_10_q, fisher_threshold, is_anno, out_file, "."+str(idx), compare_flag, flag_mis_base_0))
            jobs.append(proc)
            proc.start()
    ...
    # 
        ...
        with open(out_file, 'w') as w:
            if header_flag:
                Print_header(w, in_bam1, in_bam2, sample1, sample2, ref_fa, is_anno)
            Pileup_command(region, cmd_list, min_depth, min_variant_read, mismatch_rate_disease, mismatch_rate_normal, post_10_q, fisher_threshold, is_anno, out_file, compare_flag, positions_bed, w, flag_mis_base_0)
