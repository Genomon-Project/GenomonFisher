...
############################################################
def print_data( data, w, min_depth, mismatch_rate_disease, mismatch_rate_normal, posterior_10_quantile, fisher_threshold, min_variant_read, flag_mis_base_0, mismatch_rate_base_0, fisher_threshold_log_base_0):
    ...
    if data[ const.POS_COUNT ] == 1:
        ...
        if (
            ...
            (data[ const.POS_FISHER_SNV ] >  fisher_threshold_log or 
                (data[ const.POS_DATA2 ][ 'total_' + data[ const.POS_DATA1 ][ 'mis_base' ] ] == 0 and 
                flag_mis_base_0 == True and 
                data[ const.POS_DATA1 ][ 'mis_rate' ] > mismatch_rate_base_0 and
                data[ const.POS_FISHER_SNV ] > fisher_threshold_log_base_0)
            )
           ):
            #
            ...
            for indel_data in [ x for x in data[ data_type ].split( ',' ) if x != 'N:1.0' ]:
                ...
                if (
                    float( fisher_value) >  fisher_threshold_log or 
                        (data[ const.POS_DATA2 ][ 'indel' ][ data_type_symbol ][ bases ][ 'both' ] == 0 and
                        flag_mis_base_0 == True and
                        data[ const.POS_DATA1 ][ 'mis_rate' ] > mismatch_rate_base_0 and
                        data[ const.POS_FISHER_SNV ] > fisher_threshold_log_base_0)
                    ):
